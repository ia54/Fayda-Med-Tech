...
export default function LiensPage() {
  ...
  // Queries
  const { currentData: liensData, isFetching: isLiensLoading, isError, refetch } = useGetLiensQuery({ search: searchTerm, page, per_page: 15, status: statusFilter === "all" ? undefined : statusFilter })
  ...
  const openRecord = (record: Lien | null) => {
    ...
    setSaveError("")
    setCaseSearch("")
    setFormData({ case_id: record ? String(record.case_id) : "", provider_id: record?.provider_id ? String(record.provider_id) : "",
      lien_type: record?.lien_type || "medical", amount: record ? String(record.amount) : "", status: record?.status || "pending", notes: record?.notes || "" })
  ...
  const handleCreateLien = async () => {
    ...
    try {
      const body = { case_id: Number(formData.case_id), provider_id: formData.provider_id ? Number(formData.provider_id) : null,
        lien_type: formData.lien_type as Lien["lien_type"], amount: formData.amount,
