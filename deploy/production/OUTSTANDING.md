...
- DocuSign remains a launch blocker for electronic signing: token authentication is hardcoded to the demo OAuth endpoint, production account/base-URI consistency is unverified, and send failures can return raw provider diagnostics. Duplicate sends and out-of-order webhook state transitions need tests and remediation. Actual synthetic envelope delivery and signature completion remain unverified. Do not advertise electronic signing as ready.
